...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $check = $conexion->prepare("SELECT COUNT(*) FROM usuarios WHERE id_usuario=?");
    ...
    if ($existe == 0) {
        echo "<script>
                alert('El usuario con ID $id_usuario no existe. Verifica antes de continuar.');
                window.history.back();
              </script>";
        exit;
    }

    // Si existe, actualizar la cita
